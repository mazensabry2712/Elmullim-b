...
@section('title', 'Edit Education Level | Elmullim')
...
<div class="container">
    <div class="page-inner">
        <div class="page-header">
            <h3 class="fw-bold mb-3">Edit Education Level</h3>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            <h4 class="card-title">
                                ...
                                Edit "{{ $educationLevel->name }}"
                            </h4>
                            <div class="ms-auto">
                                <a href="{{ route('educationlevel.index') }}" class="btn btn-secondary btn-round me-2">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('educationlevel.create') }}">
                                                <i class="fas fa-plus me-2"></i>Create New
                                            </a>
                                        </li>
                                        <li><hr class="dropdown-divider"></li>
                                        <li>
                                            <form action="{{ route('educationlevel.destroy', $educationLevel) }}"
                                                  method="POST"
                                                  style="display: inline;"
                                                  class="delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="dropdown-item text-danger delete-btn" data-level-name="{{ $educationLevel->name }}">
                                                    <i class="fas fa-trash me-2"></i>Delete This Level
                                                </button>
                                            </form>
                                        </li>
                                    </ul>
                    ...
                    <div class="card-body">
                        <form action="{{ route('educationlevel.update', $educationLevel) }}" method="POST" id="education-level-form">
                            @csrf
                            @method('PUT')
                                        ...
                                        <select class="form-select @error('education_system_id') is-invalid @enderror"
                                            ...
                                            @foreach($educationSystems as $system)
                                                <option value="{{ $system->id }}"
                                                        {{ old('education_system_id', $educationLevel->education_system_id) == $system->id ? 'selected' : '' }}>
                                                    {{ $system->name }}
                                                    @if($system->country)
                                                        ({{ $system->country->name }})
                                                    @endif
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror"
                                                  ...
                                                  maxlength="1000">{{ old('description', $educationLevel->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        @enderror
                                        <small class="form-text text-muted">
                                            <span id="char-count">{{ strlen($educationLevel->description ?? '') }}</span>/1000 characters
                                        </small>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="card bg-light">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        {{ $educationLevel->educationSystem->name ?? 'N/A' }}
                                                        @if($educationLevel->educationSystem && $educationLevel->educationSystem->country)
                                                            ({{ $educationLevel->educationSystem->country->name }})
                                                        @endif
                                                    </small>
                                                </div>
                                            </div>
                                            <div class="mt-2">
                                                <small class="text-muted">
                                                    {{ $educationLevel->description ?: 'No description' }}
                                                </small>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card bg-primary text-white">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    <strong id="preview-name">{{ $educationLevel->name }}</strong>
                                                    <br>
                                                    <small class="text-white-50">
                                                        <i class="fas fa-graduation-cap me-1"></i>
                                                        <span id="preview-system">
                                                            {{ $educationLevel->educationSystem->name ?? 'N/A' }}
                                                            @if($educationLevel->educationSystem && $educationLevel->educationSystem->country)
                                                                ({{ $educationLevel->educationSystem->country->name }})
                                                            @endif
                                            ...
                                            <div class="mt-2">
                                                <small class="text-white-50" id="preview-description">
                                                    {{ $educationLevel->description ?: 'No description' }}
                                                </small>
                                            </div>
                                        </div>
                                        ...
                                        <div class="card-body">
                                            {{-- <div class="row">
                                                <div class="col-md-4">
                                                    ...
                                                    <strong>{{ $educationLevel->created_at->format('M d, Y \a\t g:i A') }}</strong>
                                                </div>
                                                <div class="col-md-4">
                                                    <small class="text-muted d-block">Last Updated</small>
                                                    <strong>{{ $educationLevel->updated_at->format('M d, Y \a\t g:i A') }}</strong>
                                                </div>
                                                <div class="col-md-4">
                                                    <small class="text-muted d-block">ID</small>
                                                    <strong>#{{ $educationLevel->id }}</strong>
                                                </div>
                                            </div> --}}
                                        </div>
                                    </div>
